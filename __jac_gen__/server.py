...
class infer(_Jac.Walker):
    ...
    def init_router(self, _jac_here_: _Jac.RootType) -> None:
        if _Jac.visit_node(self, (lambda x: [i for i in x if isinstance(i, Router)])(_Jac.edge_ref(_jac_here_, target_obj=None, dir=_Jac.EdgeDir.OUT, filter_func=None, edges_only=False))):
            ...
            _Jac.connect(left=router_node, right=ClarificationChat(), edge_spec=_Jac.build_edge(is_undirected=False, conn_type=None, conn_assign=None))
...
@_Jac.make_node(on_entry=[_Jac.DSFunc('respond', infer)], on_exit=[])
@__jac_dataclass__(eq=False)
class ClarificationChat(Chat, _Jac.Node):
    chat_type: ChatType = _Jac.has_instance_default(gen_func=lambda: ChatType.Clarification)

    def respond(self, _jac_here_: infer) -> None:

        def ask_for_clarification(message: str) -> str:
            response = 'Could you please provide more details or clarify your question?'
            return response
        _jac_here_.response = ask_for_clarification(_jac_here_.message)
